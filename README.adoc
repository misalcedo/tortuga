...
=== AssemblyScript
Some examples use AssemblyScript to compile to WASM. To build the examples, change to the `resources/asc-examples` workspace directory. Then, run `npm run asbuild`. Built examples can be found in: `resources/asc-examples/build/*.wasm`.