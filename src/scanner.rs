...
impl<'source> Scanner<'source> {
    ...
    pub fn new(code: &'source str) -> Self {
        Scanner { code, location: Location::default(), remaining: code.grapheme_indices(true) }
    }
}
...
impl<'source> Iterator for Scanner<'source> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match next_grapheme {
            None => None,
            Some((index, grapheme @ "+")) => {
                Some(Ok(Token::new(TokenKind::Plus, &self.code[index..grapheme.len()], self.location.bind())))
            },
