...
impl UnicodeProperties for char {
    fn is_pattern_white_space(&self) -> bool {
        binary_search_range_table(self, PATTERN_WHITE_SPACE)
    }

    fn is_xid_start(&self) -> bool {
        self.is_ascii_alphabetic()
            || (&'\x7f' < self && binary_search_range_table(self, XID_START_TABLE))
    }

    fn is_xid_continue(&self) -> bool {
        self.is_ascii_alphanumeric()
            || &'_' == self
            || (&'\x7f' < self && binary_search_range_table(self, XID_CONTINUE_TABLE))
    }
}

fn binary_search_range_table(c: &char, table: &Table) -> bool {
    table
...
static PATTERN_WHITE_SPACE: &Table = &[
    ('\u{0009}', '\u{000D}'),
    ('\u{0020}', '\u{0020}'),
    ('\u{0085}', '\u{0085}'),
    ('\u{200E}', '\u{200F}'),
...
static XID_CONTINUE_TABLE: &Table = &[
    ('\u{30}', '\u{39}'),
    ('\u{41}', '\u{5a}'),
    ('\u{5f}', '\u{5f}'),
    ('\u{61}', '\u{7a}'),
